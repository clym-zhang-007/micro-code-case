...
try:
    ...
    print("初始化 OpenAI 客户端失败，请检查 API Key 与 OPENAI_BASE_URL")
    print(f"错误信息: {e}")
    exit(1)


def _parse_json_from_llm(text: str):
    """解析模型返回的 JSON；若包裹在 markdown 代码块中则先剥离。"""
...
# 基于 prompt 生成文本（chat.completions，OpenAI 协议）
def get_completion(prompt, model=None):
    messages = [{"role": "user", "content": prompt}]
    m = model or CHAT_MODEL
    # 通过 extra_body 透传网关扩展参数；默认关闭思考模式
    ...
    try:
        ...
        raise RuntimeError(f"chat.completions 调用失败（model={m}）: {e}") from e
...
class QueryRewriter:
    ...
    def auto_rewrite_and_execute(self, query, conversation_history="", context_info=""):
        ...
        # 根据识别结果调用相应的改写方法
        query_type = result.get("query_type", "") if isinstance(result, dict) else ""
