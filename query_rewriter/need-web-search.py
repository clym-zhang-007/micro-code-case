...
# 与 embedding_faiss/embedding-faiss.py 相同：脚本目录下 .env + OpenAI 兼容客户端
...
CHAT_MODEL = os.getenv("CHAT_MODEL", "kimi-k2.5")
...
if not _api_key:
    print("请设置环境变量 DASHSCOPE_API_KEY 或 OPENAI_API_KEY（可在 query_rewriter/.env 中配置）。")
    exit(1)
try:
    client = OpenAI(api_key=_api_key, base_url=OPENAI_BASE_URL)
except Exception as e:
    print("初始化 OpenAI 客户端失败，请检查环境变量 DASHSCOPE_API_KEY（或 OPENAI_API_KEY）与 OPENAI_BASE_URL。")
    print(f"错误信息: {e}")
    exit(1)


# 基于 prompt 生成文本（OpenAI chat.completions 协议）
def get_completion(prompt, model=None):
    ...
    if choice is None or not getattr(choice, "message", None):
        raise RuntimeError(f"网关未返回有效 choices: {response}")
    content = choice.message.content
    if content is None:
        raise RuntimeError(f"网关返回 content 为空: {choice}")
...
def _extract_need_web_search_fallback(text: str):
    """
    在 JSON 解析失败时，从原始文本中兜底提取 need_web_search 布尔值。
    例如：need_web_search: true / "need_web_search": false
    """
    if not text or not isinstance(text, str):
        return None
    m = re.search(r'need_web_search"\s*:\s*(true|false)|need_web_search\s*:\s*(true|false)', text, re.IGNORECASE)
    if not m:
        return None
...
class WebSearchQueryRewriter:
    ...
    def identify_web_search_needs(self, query, conversation_history=""):
        ...
        try:
            ...
            fallback_bool = _extract_need_web_search_fallback(response)
            if fallback_bool is not None:
                return {
                    "need_web_search": fallback_bool,
                    "search_reason": "JSON解析失败，使用文本兜底提取。",
                    "confidence": 0.6
                }
            return {
